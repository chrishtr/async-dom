...
JavaScript uses a single-threaded, event loop-based programming model. In addition, it does not distinguish between code and data. These together make it very hard or impossible to share objects (other than SharedArrayBuffer) between threads, and just as importantly or more importantly, share *code* between threads. A C++ program cah easily share all of its code across all threads, because it is generally read-only memory, but that is no the case with JavaScript.

JavaScript can be run on multiple threads within the same process, but these threads have performance and API constraints that are much more like processes than C++ threads. Communication between threads generally happens with event passing and structured data cloning, as opposed to synchronization primitives and shared memory. This makes it much harder to delegate certain tasks like I/O to auxiliary threads, because it's difficult to get the data back from them in an form other than raw bytes, so even if browser-provided I/O methods run on a thread, the processing into convenient JavaScript objects is hard to do in that way.

In addition, the DOM is available on on the main thread, and is represented as normal JS objects exposed directly to script. Thus, JavaScript's difference from C++ is one reason why DOM isn't/is hard to put on multiple threads. Another reason, however, is that it's hard to program a UI with model or view state spread across multiple threads. Recall that this latter reason is als the case for native applications - they often run auxilary tasks on other threads, but the model and view state of the application are almost always maintained on one thread. But if that's the case, is it just as easy to make the DOM efficient as a native application UI? The answer is, in principle, yes. If a web browser is optimized sufficiently for this situation, and a web UI implements exactly the same UX as a non-web app, it should be possible to get close to the same performance and responsiveness. (Close, but not exact, because of the inherent overhead of interpreted languages, the event model, the DOM APIs, and a few other pieces of constant-factor overhead.)

However, this is not really how the web APIs are meant to be used, and therefore how web apps are typically structured in practice. One difference is that web app UIs very often represent a large amount of semi-structured information that is some hybrid between a document an an "app". This is also related to the notion of web as a collection of information pages, as opposed to an "app" which does a collection of related things and has its own UI system. A second is that the DOM is architecturally an inversion-of-control system. The developer does work to update the DOM to a new state, but beyond that is not able to give any instructions to the browser in terms of the best way to cause that udate to result in changes on the screen. This is as opposed to a direct API, where it is obvious to the developer and the OS what is desired and what needs to happen next.

** Opportunity or footgun?

This inversion-of-control is the reason the web is often described as having many "performance footguns", meaning APIs that result in a lot more browser work than the developer may have realized.
